...
def preprocess_zonals(zonal_list):
    ...
    # print("Processing zonals...",zonal_list)
    for z in zonal_list:
        # print("Processing zonal:", z)
        try:
...
def get_filtered_workspace_list(book_date, current_employee_number, get_booking_for_web_list, zonal_list):
    # print("zonal list....................", zonal_list)
    if isinstance(book_date, str):
        book_date = datetime.strptime(book_date, "%Y-%m-%d")
    seat_to_zonals = preprocess_zonals(zonal_list)
    # print("Processed zonals:", seat_to_zonals)
    for seat_id, details in list(seat_to_zonals.items())[:10]:
     print(seat_id, details)
    ...
    for seat in get_booking_for_web_list["asscocicateInfoList"]:
        ...
        # print("seat::::::::::::::::::::::",seat)
        # print("seat_to_zonals.get(workspace_id)", seat_to_zonals.get(workspace_id))
        zonals = seat_to_zonals.get(workspace_id, []) #-------------->this is faaling
         # print("zonals::::::::::::::::::",zonals)
        for z in zonals:
            print("z['from_date']", z["from_date"])
            print("z['to_date']", z["to_date"])
            print("book_date", book_date)
            if z["from_date"] <= book_date <= z["to_date"]:
                print(":::::::::::::::::::::inside the zonal")
                if str(current_employee_number) not in z["emp_set"]:
                    print(":::::::::::::::::::::you cannot book this zonal====TRUE")
...
async def get_available_seats(building: str, floor: str, date: str) -> dict:
    ...
    try:
        ...
        # print("line 808 zonal_list", zonal_list)
    except Exception as e:
        print(f"Error fetching zonal information: {e}")
        return {"error": "Failed to retrieve zonal information."}
    try:
        booking_date = datetime.strptime(date, "%Y-%m-%d")
    except ValueError:
        print("Invalid date format provided. Expected YYYY-MM-DD.")
        return {"error": "Invalid date format. Please use YYYY-MM-DD."}

    employee_id = _current_employee_id.get()
    if not employee_id:
        print("Employee ID not found in context. Cannot get available seats.")
    ...
    # print("line 722 get_floor_booking_info::::::::::::::::::",booking_info.json())
    ...
    with open("result_output.json", "w", encoding="utf-8") as f:
        json.dump(seats_with_zonal_info, f, indent=2, ensure_ascii=False)
    # print("line 808 seats_with_zonal_info", seats_with_zonal_info)
    available_seats = [
    {"seat_no": seat['worspaceName'], "workspace_id": seat['workspaceId']}
    for seat in seats_with_zonal_info
    if (seat.get("workspaceStatusId") == 1 and seat.get("youCannotBookThisZonnal")==False and
        seat.get('worspaceName') is not None and
        str(seat.get('worspaceName')).strip() != "")]
    
    if available_seats:
        top_6_seats = available_seats[:10]
        print("line 813 top_6_seats", top_6_seats)
...
app = FastAPI(title="LangGraph Arena Bosch Chatbot API", version="live8")
...
known_locations = {
    ...
    "KT1": "Coimbatore SEZ", "KT2": "Coimbatore SEZIL", "EC1": "Electronic City", "PBP": "Pune",
...
def get_system_prompt() -> HumanMessage:
    ...
    prompt_content = f"""
    You are a helpful assistant designed to book, cancel seats and view bookings history.
    ...
    **Important Rules:**
    ...
    - **Seat Number Interpretation:**
      - If the user provides a seat number **without a prefix** (e.g., "736", "054", "568"), do **not** assume it's a workspace.
      - Ask the user:  
        > "Is this seat for a *workspace* or a *cabin*?"
      - Based on the user's answer:
        - **Workspace**: Format as `L<FloorNumber>-<SeatCode>`. You must have the floor number from the user (ask if missing).
        - **Cabin**: Use the seat number as-is (no formatting, e.g., "568").
      - If the user provides a seat number **with a prefix** (e.g., "L1-736", "L0034") assume it as a workspace and do not ask user again.

    - **Workspace Formatting Rules**:
          - Users may provide Workspace numbers in any format (e.g., “l0 bo34”, “L12bo24”, “l3a054”).Always convert and return seat numbers in the standard format:L<FloorNumber>-<SeatCode>
         For example:
                      “l0 b034” → L0-B034
                       “l1 001” → L1-001
                       “l12024” → L12-024
    ...
    - **Cabin Formatting Rules**:
        - Do **not** apply any formatting. Use directly:  
          Examples: “568”, “664”, “723”

    - **Always Confirm Booking Type** if the input is ambiguous.

    - **Allowed Booking Days**: Bookings are allowed for **today and up to the current week only**.

    - **Communication**: Be friendly, clear, and guide the user step-by-step. Do not show step numbers to the user.
    """
...
async def chat(
    ...
    try:
        ...
        # employee_id=11446688

        # Karunakaran ec360
        # employee_id=35815608  
...
@app.get("/")
async def root():
    return {"message": f"LangGraph Arena Bosch Chatbot API: live8"}
